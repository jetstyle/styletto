...
module.exports = function( params, callback ) {
    ...
    if ( ( params.stylus === undefined ) || params.stylus === false ) {
        ...
        if ( params.stylus.mixins ) {

            if ( params.stylus.mixins instanceof Array ) {

                for ( var m = 0; m < params.stylus.mixins.length; m++ ) {


                    var pathToMixin = validatePath( params.stylus.mixins[m], params.path );



                    if ( pathToMixin ) {

                        params.stylus.mixins[m] = pathToMixin;

                    }

                    else {

                        err += 'Error! '.red + 'Wrong path of type of file in mixin: "' + params.mixins.stylus[m] + '".\n';
                ...
                err += 'Error!'.red + 'stylus.mixins must be array with paths to mixins.\n';
